...
LOG_FILE = "TG_MSG.log"  # 新增 Log 檔案名稱
...
def send_telegram(message):
    ...
    # 記錄到 Log Buffer
    LOG_BUFFER.append(f"[{datetime.now().strftime('%H:%M:%S')}] 🤖 Bot: {message[:100]}..." if len(message) > 100 else f"[{datetime.now().strftime('%H:%M:%S')}] 🤖 Bot: {message}")
    ...
    if os.path.exists(LOG_FILE):
        try:
            with open(LOG_FILE, "r", encoding="utf-8") as f:
                content = f.read()
                # 切割舊的 Header 和 Logs，只保留 Logs 部分
                if separator in content:
                    old_logs = content.split(separator)[1]
                else:
                    # 如果格式不符（第一次加入此功能），保留所有內容作為舊 log
                    old_logs = content
        except: pass
...
def get_sprint_status(user_data):
    """
    計算目前是否落後於 N2 衝刺計畫。
    邏輯更新：若難度已達標 (>= 4.0)，則不再顯示衝刺落後訊息，改為無限模式。
    """
    stats = user_data["stats"]
    current_difficulty = float(stats.get("current_difficulty", 2.0))

    # 1. 判斷是否已經是大神 (N2 以上)
    # 如果已經 >= 4.0，直接跳過衝刺邏輯，回傳無限模式訊號
    ...
    status_msg = ""
    if days_diff >= 5:
        status_msg = f"🔥 超前進度：你比預期快了 {days_diff} 天！保持這種神速！"
    elif days_diff <= -5:
        status_msg = f"⚠️ 落後警報：你已經落後計畫 {abs(days_diff)} 天了！距離 N2 越來越遠囉？"
    else:
        status_msg = f"✅ 進度正常：穩步邁向 N2 中，請繼續保持。"
...
def assess_user_level(history_logs, specific_request=None):
    ...
    print("🧠 AI 正在進行全盤能力評估...")
    LOG_BUFFER.append(f"[{datetime.now().strftime('%H:%M:%S')}] 🧠 AI Action: 執行能力評估 ([CH])")
...
def ai_correction(user_text, translation_history, progress_status):
    ...
    # 🔥 恢復原始的高細膩度 Prompt，並加入雙向識別邏輯與更嚴格的文法檢查
    prompt = f"""
...
def process_data():
    ...
    try:
        ...
        for item in response["result"]:
            ...
            LOG_BUFFER.append(f"[{msg_time}] 👤 User: {text}")
...
def run_daily_quiz(vocab, user):
    ...
    if is_new_day:
        ...
        if user["stats"]["last_quiz_date"] == "2000-01-01":
            ...
            if answer_rate >= 0.8: # 表現優異
                ...
                if not is_infinite_mode:
                    sprint_warning = f"特別注意：請引用「衝刺狀態數據 ({sprint_msg})」來警告她 (例如：我們已經落後 X 天了，這時候睡覺對得起你的 N2 報名費嗎？)。"
                else:
                    sprint_warning = "請提醒她：無限之路不進則退，不要鬆懈了！"
        ...
        # 根據模式切換 Prompt 中的衝刺資訊
        sprint_info_block = ""
        if is_infinite_mode:
            sprint_info_block = "目前為【無限挑戰模式】，沒有進度壓力，目標是超越極限。"
        else:
            sprint_info_block = f"""
            【衝刺狀態】
        ...
        **🎯 目標難度等級：{current_difficulty:.1f}**
        ...
        【情緒與開場】
        {emotion_prompt}
        請在開場白中根據目前的進度狀態 (落後、超前或無限挑戰) 展現出對應的教練態度。
