...
def process_data():
    ...
    user_data = load_json(USER_DATA_FILE, {
        "stats": {
            ...
            "current_period_answer_count": 0, # ✅ 改名：本期累積答題數
            "last_update_id": 0
        },
        "pending_answers": "",
        "translation_log": []
    ...
    # 資料結構遷移 (若是舊檔案，補上新欄位)
    if "current_period_answer_count" not in user_data["stats"]:
        # 嘗試從舊欄位繼承，如果沒有則歸零
        user_data["stats"]["current_period_answer_count"] = user_data["stats"].get("yesterday_answers_count", 0)
    ...
    try:
        ...
        today_answers_detected = 0 # 這次執行偵測到的新回答數
        ...
        for item in response["result"]:
            ...
            if not text.startswith("/"):
                ...
                today_answers_detected += lines_count # 累加到本次偵測數
        ...
        # === 關鍵修改：即時更新答題數 ===
        # 不論日期是否變更，只要有新回答，就加入「本期累積」
        if today_answers_detected > 0:
            user_data["stats"]["current_period_answer_count"] += today_answers_detected
            is_updated = True
...
def run_daily_quiz(vocab, user):
    ...
    # 2. 判斷情緒 Prompt
    # 邏輯：第一次執行(Count=1) 或 初始化後(Date=2000) -> 天使模式
    ...
    # ✅ 關鍵：讀取「本期累積答題數」
    answers_given = user["stats"].get("current_period_answer_count", 0)
    ...
    try:
        ...
        if response.text:
            ...
            # ✅ 關鍵：出題完畢，本期答題數歸零，準備計算下一期
            user["stats"]["current_period_answer_count"] = 0
