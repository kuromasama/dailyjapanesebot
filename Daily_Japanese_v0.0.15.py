...
def send_telegram(message):
    ...
    # 記錄到 Log Buffer
    log_content = message[:100] + "..." if len(message) > 100 else message
    log_to_buffer("🤖 Bot", log_content)
...
def get_sprint_status(user_data):
    ...
    status_msg = ""
    if days_diff >= 5:
        status_msg = f"🔥 超前進度：你比預期快了 {days_diff} 天！"
    elif days_diff <= -5:
        status_msg = f"⚠️ 落後警報：你已經落後計畫 {abs(days_diff)} 天了！"
    else:
...
def ai_correction(user_text, translation_history, progress_status):
    ...
    2. **📈 進度評估**：比較歷史紀錄，判斷是否有進步？給予鼓勵或警惕。
    
    3. **🎯 深度批改 (不僅是改錯)**：
       - 若是日文輸入：
         - 標示具體的文法錯誤 (❌) 並修正 (✅)。
         - **特別點評：** 針對「文法結構」與「用詞精準度」給予專業建議，而不只是改單字。
    ...
    6. **🚨 【系統指令：錯誤收錄】(非常重要)**：
       - 如果使用者的回答中有**明顯的單字錯誤**或**文法誤用**，請在回應的**最後面**，附上一個 JSON 區塊，格式如下：
       ```json
       {{
         "mistakes": [
            {{ "term": "誤用詞/正確詞", "type": "word", "meaning": "詞意" }},
            {{ "term": "文法項目", "type": "grammar", "meaning": "文法解說" }}
         ]
       }}
       ```
       - 如果完全正確，則不需要附上此區塊。
       - 請注意：`type` 只能是 `word` (單字) 或 `grammar` (文法)。
...
def process_data():
    ...
    try:
        ...
        for item in response["result"]:
            ...
            found_count += 1
            msg_time = datetime.fromtimestamp(item["message"]["date"]).strftime('%H:%M:%S')
        ...
        if not is_fresh_start and pending_correction_texts:
            ...
            try:
                ...
                if json_match:
                    ...
                    mistakes_data = json.loads(json_str)
                    
                    final_msg_text = raw_result.replace(json_match.group(0), "").strip()
                    log_to_buffer("⚙️ Mistake", f"Found mistakes JSON: {json_str}")
                    
                    if "mistakes" in mistakes_data:
                        mistake_log_list = []
                        for m in mistakes_data["mistakes"]:
                            term = m.get("term", "")
                ...
                log_to_buffer("⚠️ Err", f"Mistake parsing failed: {e}")
            ...
            for w in vocab_data["words"]:
                if normalize_text(w["kanji"]) in text_for_search:
                    if normalize_text(w["kanji"]) not in mistaken_terms:
                        if w.get("count", 1) > 1:
                            w["count"] = max(1, w["count"] - 2) # 答對獎勵：快速下降
...
def run_daily_quiz(vocab, user):
    ...
    if is_new_day:
        ...
        if user["stats"]["last_quiz_date"] == "2000-01-01":
             emotion_prompt = f"初次見面，請介紹 N2 衝刺計畫。目前 Lv{current_difficulty}。"
        else:
            if answer_rate >= 0.8:
                increase = 0.2 if answer_rate < 1.0 else 0.3
                current_difficulty += increase
                difficulty_adjustment_msg = f"🔥 難度升至 Lv{current_difficulty:.1f}！"
                emotion_prompt = f"昨日表現優異 ({main_score}/10)。請大力稱讚。"
            elif answer_rate >= 0.4:
                if not is_infinite_mode and current_difficulty < expected_diff:
                    emotion_prompt = f"昨日表現普通。警告：落後進度！"
                else:
                    emotion_prompt = f"昨日表現普通。維持 Lv{current_difficulty:.1f}。"
            else:
                current_difficulty = max(1.0, current_difficulty - 0.3)
                difficulty_adjustment_msg = f"📉 降至 Lv{current_difficulty:.1f}。"
                sprint_warn = f"引用衝刺狀態 ({sprint_msg}) 警告落後！" if not is_infinite_mode else "無限之路不可鬆懈！"
                emotion_prompt = f"昨日表現偷懶 ({main_score}/10)。開啟幽默情勒模式。{sprint_warn} 並提到 {difficulty_adjustment_msg}。"
        
        user["stats"]["current_difficulty"] = float(f"{current_difficulty:.1f}")

        print(f"🤖 生成每日必修 - Lv{current_difficulty:.1f}...")
        ...
        **🎯 目標難度：{current_difficulty:.1f}**
        - 基礎 (Lv{base_level}): {base_desc} ({(1-decimal_part)*100:.0f}%)
        - 進階 (Lv{base_level+1}): {next_desc} ({decimal_part*100:.0f}%)
        ...
        **注意：若是標記 (文法) 的項目，請務必設計出能展現該文法接續與用法的句子。**
        ...
        【輸出格式要求】
        1. 全繁體中文。
        2. 嚴禁 Markdown 標題 (#)。使用 Emoji 區隔。
        3. 結構：
           - Part 1: 題目卷 (含開場、10題)。**不要**給答案。
           - 分隔線: `|||SEPARATOR|||`
           - Part 2: 解答卷 (含參考答案與解析)。
        """
    ...
    # ================= Scenario B: Bonus 無限挑戰 =================
        ...
        bonus_difficulty = base_diff + (bonus_count // 3) * 0.5 + 0.5 
        
        print(f"🤖 生成 Bonus - Lv{bonus_difficulty:.1f}...")
        
        prompt = f"""
        你是日文 N2 斯巴達教練。使用者主動挑戰 Bonus。
        用「充滿誘惑力與挑戰性」的語氣開場 (魔鬼教練的露齒一笑)。
        
        **Bonus 難度：{bonus_difficulty:.1f}**
