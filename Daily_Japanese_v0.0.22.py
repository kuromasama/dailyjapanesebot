...
def get_sprint_status(user_data):
    ...
    diff = current_difficulty - START_DIFFICULTY
    expected_growth = (TARGET_DIFFICULTY - START_DIFFICULTY) * progress_ratio
    actual_growth = current_difficulty - START_DIFFICULTY
...
def ai_correction(user_text, translation_history, progress_status):
    ...
    # 🔥 斯巴達教練 Prompt - v0.0.23 創意鎖定與多樣化生成
    prompt = f"""
    ...
    3. **🎯 深度批改 (逐句檢討)**：
       - 請將使用者的輸入拆解成獨立的句子（如果有多句），並**逐句**進行批改與點評。
       - 標示具體的文法錯誤 (❌) 並修正 (✅)。
    ...
    7. **📊 【逐句評分與教練短評】(v22 核心升級)**：
       ...
       - **評分標準**：
         - **9.0~10.0 (神級)**：文法完美，語感道地，使用了進階語彙/換句話說。
