...
def send_telegram(message):
    ...
    clean_msg = clean_msg.replace("<br>", "\n").replace("<br/>", "\n").replace("<br />", "\n")
...
def process_data():
    ...
    # 初始化 execution_count (防呆)
    if "execution_count" not in user_data["stats"]:
        user_data["stats"]["execution_count"] = 0
...
def run_daily_quiz(vocab, user):
    ...
    # 2. 判斷情緒 Prompt
    is_first_run = user["stats"]["last_quiz_date"] == "2000-01-01"
    ...
    if is_first_run:
        ...
        if answer_rate >= 0.8:
            ...
            emotion_prompt = f"昨日表現：回覆 {answers_given}/{questions_given} 題。狀態：偷懶！請開啟【情勒模式 😈】。"
    ...
    【系統資訊】
    這是第 {exec_count} 次執行特訓。
    
    【情緒與開場】
    {emotion_prompt}
    請在開場白中明確提到：「這是我們的第 {exec_count} 次特訓！」。
