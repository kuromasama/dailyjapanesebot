...
def ai_correction(user_text, translation_history, progress_status):
    ...
    # 🔥 斯巴達教練 Prompt - v0.0.21 新增評分與獎勵機制
    prompt = f"""
    ...
    6. **🚨 【系統指令：錯誤收錄】(非常重要)**：
       - 若有錯誤，請在回應最後的 JSON 中列出。
       
    7. **📊 【評分與挑戰獎勵】(v21新增)**：
       - 請針對使用者的翻譯給予一個 **0.0 ~ 10.0** 的評分。
       - **計分標準**：
         - **準確度 (Accuracy)**：文法、單字正確無誤。
         - **流暢度 (Fluency)**：語氣自然道地。
         - **🔥 挑戰加分 (Challenge Bonus)**：(特別針對中翻日)
           - 如果使用者使用了**歷史紀錄中不常出現的高級詞彙**。
           - 如果使用者使用了**進階文法** (N2/N3) 而非僅用簡單句。
           - 如果使用者使用了**精彩的「換句話說」**。
           - 若有上述情況，請給予高分 (9.0~10.0) 並在評語中特別稱讚！
       - 若只是普通的正確，給予 7.0~8.5 即可。
       - 若有錯誤，請低於 6.0。
       - 請在回應開頭以醒目的方式顯示分數：
         「🏆 **本次評分：9.5 / 10.0** (🔥 使用了進階語彙，挑戰成功！)」
        ...
        if not is_fresh_start and pending_correction_texts:
            ...
            try:
                ...
                if json_match:
                    ...
                    # 2. 處理雙軌難度調整 (Assessment Score)
                    if "assessment" in parsed_data:
                        assess = parsed_data["assessment"]
                        q_type = assess.get("type", "") 
                        score = float(assess.get("score", 0.0))
                        
                        target_key = "difficulty_cn_jp" if q_type == "CN_TO_JP" else "difficulty_jp_cn"
                        
                        if q_type in ["CN_TO_JP", "JP_TO_CN"]:
                            if score >= 9.0: # 神級發揮，挑戰成功 (+0.1)
